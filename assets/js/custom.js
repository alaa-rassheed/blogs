(function ($) {
    ...
    function updateDate() {
        ...
        // Format for day name
        var dayFormatter = new Intl.DateTimeFormat('ar-EG', {
            weekday: 'long'
        });

        // Format for day number and year
        var numberFormatter = new Intl.DateTimeFormat('en-US', {
            day: 'numeric',
            year: 'numeric'
        });
        ...
        // Safely get day number and year
        var formattedParts = numberFormatter.formatToParts(now);
        var dayNumber = formattedParts.find(part => part.type === 'day')?.value || '';
        var year = formattedParts.find(part => part.type === 'year')?.value || '';
        ...
        $('#date-display').text(dayName + ' ' + dayNumber + ' ' + monthName + ' ' + year);
    }
    ...
    // Update the date immediately and schedule the next update
    updateDate();
    scheduleNextUpdate();
})(jQuery);

$(window).on('scroll', function () {
    var header = $('.life-style');
